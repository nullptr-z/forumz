...
export default () => {
  ...
  const fetchFriendList = async () => {
    const resp = await axios.get(`/user/getFriendsListById?userId=${userId}`)
    console.log("【 resp 】==>", resp);
    setFriends(resp?.data || [])
  }

  const addFriendOverlayShow = () => {
    setOverlayVisible(!overlayVisible)
  }

  return (
    <React.Fragment>
      <AddFriend readOnly visible={overlayVisible} onClose={addFriendOverlayShow} />
      <div className="flex h-screen antialiased text-gray-800">
        <div className="flex flex-row h-full w-full overflow-x-hidden">
          <div className="flex flex-col py-8 pl-6 pr-2 w-64 bg-white flex-shrink-0">
            ...
            <button
              onClick={addFriendOverlayShow}
              className="flex flex-row justify-center bg-slate-100 hover:bg-gray-300 rounded-xl p-2 text-center">
              <p>+</p> 添加好友
            </button>
          </div>
